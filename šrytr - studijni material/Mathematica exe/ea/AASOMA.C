...
void AASoma P(( double,double,double,int,int,int,double, doublep_nt));

void AASoma( double Mass,double Step,double PRT, int NP, int D, int Migrations,double AcceptedError,doublep_nt list2 )
{
   double	ckl1;
	int 		i,j,smc2;//l,n,neval;
	...
	for(l=0;l<Migrations;l++)
   	{
		...
		//Migration
		for(smc2=0;smc2<PNP-3;smc2++)
      	{
			for(n=0;n<PD;n++)
				setPopulation(n,PNP-1,getPopulation(n,smc2));
			for(i=0;i<PNP-3;i++)
		  		{
				if(i!=smc2)
					{
      	         ...
      	         for(j=0;j<PD-1;j++)
	      				{
		      	   	setPopulation(j,PNP-3,getPopulation(j,smc2)+(getPopulation(j,i)-getPopulation(j,smc2))*ckl1*PRTV[j]);
				      ...
				      neval++;
						CostValue(PNP-3);
						if(getPopulation(PD-1,PNP-3)<getPopulation(PD-1,PNP-1))
							{
							for(n=0;n<PD;n++)
								setPopulation(n,PNP-1,getPopulation(n,PNP-3));
							}
						}
					}
				}
			for(n=0;n<PD;n++)
				setTMPPopulation(n,smc2,getPopulation(n,PNP-1));
         }
		for(j=0;j<PNP;j++)
			for(n=0;n<PD;n++)
				setPopulation(n,j,getTMPPopulation(n,j));
	   WorstIndividual();
		BestIndividual();
