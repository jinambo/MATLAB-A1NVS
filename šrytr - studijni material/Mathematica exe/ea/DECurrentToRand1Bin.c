...
void DECurrentToRand1Bin P(( double,double, double, int, int, int,doublep_nt));

void DECurrentToRand1Bin( double F,double CR, double K, int NP, int D, int Generations,doublep_nt list )
{
	int i,j,V1,V2,V3;
   double	ckl1;
	      ...
	      for(j=0;j<PD-1;j++)
	      	{
				if((random (1000)/1000.0)<CR || j==PD-1)
		         setPopulation(j,PNP-3,getPopulation(j,i)+K*(getPopulation(j,V3)-getPopulation(j,i))+F*(getPopulation(j,V1)-getPopulation(j,V2)));
				else
